export const FRONTEND = {
  title: "Frontend",
  items: [
    "Javascript",
    "HTML",
    "React",
    "CSS",
    "Typescript",
    "Inertia",
    "PHP",
    "Tailwind",
  ],
};

export const BACKEND = {
  title: "Backend",
  items: ["Node", "Laravel", "Express", "MySQL"],
};

export const TOOLS = {
  title: "Tools",
  items: ["Git", "Figma", "GitHub"],
};

export const ABOUT = [
  {
    header: "A Closer Look",
    content:
      "I’m an IT student from the Philippines focused on web development. Currently working on both frontend and backend skills to become a Full-Stack Developer. Building practical projects while studying to gain more knowledge and apply it into working applications.",
  },
  {
    header: "Academic Journey",
    content:
      "I’m currently in my third year at Pamantasan ng Lungsod ng Pasig working toward my bachelor’s degree. At the same time, I’m learning React and Laravel  to deepen my knowledge in full-stack web development.",
  },
  {
    header: "Hobbies & Interests",
    content:
      "Beyond coding, I find relaxation in playing the guitar and watching movies, These help me unwind and gain diverse perspectives.",
  },
  {
    header: "Future Plans",
    content:
      "I'm looking forward to expanding my skill set by diving into Java Spring Boot and .NET, which will enhance my existing knowledge and open new opportunities in a more backend centric role.",
  },
];
